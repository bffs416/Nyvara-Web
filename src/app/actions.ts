...
export async function processBriefAction(values: BriefFormValues): Promise<{ success: boolean; data?: Project; error?: string; }> {
  ...
  try {
    ...
    if (values.nit) {
      try {
        const filePath = path.join(process.cwd(), 'src', 'lib', 'cronogramas.json');
        const fileContent = await fs.readFile(filePath, 'utf-8');
        const data = JSON.parse(fileContent);

        const clientIndex = data.clients.findIndex((c: any) => c.nit === values.nit);

        if (clientIndex !== -1) {
          data.clients[clientIndex].projects.push(newProject);
          await fs.writeFile(filePath, JSON.stringify(data, null, 2), 'utf-8');
          console.log(`Saved new project to cronogramas.json for client ${values.nit}`);
        } else {
          console.warn(`Client with NIT ${values.nit} not found in cronogramas.json`);
        }
      } catch (fsError) {
